...
def get_args_parser():
    ...
    parser.add_argument('--hcc_axis', type=str, default='h', help='"h" or "w"')
    parser.add_argument('--hcc_padding', type=str, default='circular', help='"circular" or "zero"')
...
def main(args):
    ...
    if args.tuning_method in ('conv', 'adapter', 'hcc'):
        ...
        # 2) Adapter implementations (return residual ONLY)
        if args.tuning_method in ('conv', 'adapter'):
            ...
            from models.hcc_adapter import HartleyCosineConv2d

            def make_adapter(ch):
                return HartleyCosineConv2d(
                    h=args.hcc_h, num_shifts=args.hcc_M,
                    axis=args.hcc_axis, padding=args.hcc_padding,
                    per_channel=args.hcc_per_channel, use_center=args.hcc_use_center
                )

        # 3) Attach adapters via forward hooks (residual-parallel on block output)
    ...
    # Move to device BEFORE profiling so CUDA stats (if any) make sense
    model.to(device)
